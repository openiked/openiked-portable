...
#define	IPSEC_DEFAULT_EMBRYONIC_SA_TIMEOUT	300	/* 5 minutes */
...
#define	TDBF_DELETED		0x00040	/* Deleted */
...
#define	TDBF_GREHIDE		0x40000	/* GRE encapsulation */
#ifndef GENUOS
	/*
	 * tdb_flags may be changed on multiple cpus, we moved TDBF_PFSYNC to
	 * tdb_pfsync, where it is always changed under PF_LOCK. TDBF_PFSYNC_RPL
	 * is not used at all, so we removed.
	 */
...
#define	TDBF_AUTH_SWITCH	0x200000 /* Already tried to switch auth before */
...
#define TDBF_BITS ("\20" \
	...
	volatile u_int	tdb_sync;	/* PFsync pending */
	struct pfsync_msg tdb_sync_msg;	/* if tdb_sync == 1, this msg will be sent */

	struct timeout	tdb_timer_tmo;
	struct timeout	tdb_first_tmo;
	struct timeout	tdb_stimer_tmo;
	struct timeout	tdb_sfirst_tmo;
	struct task	tdb_timer_task;
	struct task	tdb_first_task;
	...
	u_int64_t	tdb_bitmap;	/* Old, small window */
	u_int64_t	tdb_rpl;	/* Replay counter */
#define TDB_REPLAYWASTE	32
	u_int32_t	tdb_replaymax;	/* Size of ringbuffer in bits */
	u_int32_t	*tdb_seen;	/* Anti-replay window */
	...
	int		tdb_cpuid;	/* preferred CPU for sending */

	u_int		tdb_carpgen;		/* CARP generation count */

	struct rtcache	**tdb_rtc;		/* outgoing route cache */
	struct tdb_cache	**tdb_tcnext;	/* next outgoing tdb cache */
...
extern u_int32_t ipsec_gen;
extern int tdb_replay_window;
extern int tdb_replay_disable;
...
/* XXX note that we read the ip_mtudisc and time_second globals ! */
#define TDB_NEED_FRAGMENT(tdb, ip) \
    (ip_mtudisc && ((ip)->ip_off & htons(IP_DF)) && (tdb)->tdb_mtu && \
...
uint32_t reserve_spi(u_int, u_int32_t, u_int32_t, union sockaddr_union *,
		union sockaddr_union *, u_int8_t, int, int *);
...
void	tdb_delete_deferred(struct tdb *);
...
#define TDB_TRACE_RECORD(y, f, l, op) do { 				\
	...
	if (x) { 							\
		...
		if (tdb_trace_level > 1) 				\
			printf("%s: line %d tdb %p: cnt %d %d", f, l, 	\
			    x, x->tdb_refcnt.refs, op); 		\
	} 								\
	} while(0)
#define tdb_ref(x) ({							\
		struct tdb *y;						\
...
void	tdbc_update_locked(struct tdb_cache *, struct tdb *);
int	tdbc_equal(struct tdb_cache *, struct tdb *);
struct	tdb *tdbc_ref(struct tdb_cache *);
int	tdb_init(struct tdb *, u_int16_t, struct ipsecinit *);
int	tdb_walk(u_int, int (*)(struct tdb *, void *, int), void *);

void	tdb_apply_reverse(struct tdb *tdb, void (*)(struct tdb *,
		struct tdb *, void *), void *);
int	tdb_add_flow(struct tdb *);
int	tdb_del_flow(struct tdb *);
...
int	esp_input_cb(struct tdb *, struct mbuf *, int, int, int, int);
int	esp_output(struct mbuf *, struct tdb *, struct mbuf **, int, int);
int	esp_output_cb(struct tdb *, struct mbuf *, int, int);
...
int	ipcomp_input_cb(struct tdb *, struct mbuf *, int, int, int);
int	ipcomp_output(struct mbuf *, struct tdb *, struct mbuf **, int, int);
int	ipcomp_output_cb(struct tdb *, struct mbuf *, int, int, int);
...
int	checkreplaywindow64(struct tdb *, u_int32_t, u_int32_t *, int);

/* Packet processing */
int	ipsp_process_packet(struct mbuf *, struct tdb *, int, int);
int	ipsp_process_enqueue(struct mbuf *, struct tdb *, int);
int	ipsp_process_done(struct mbuf *, struct tdb *);
struct	tdb *ipsp_spd_lookup(struct mbuf *, int, int, int *, int,
	    struct tdb *, struct inpcb *, u_int32_t, struct tdb *);
struct	tdb *ipsp_spd_inp(struct mbuf *, int, int, int *, int,
	    struct tdb *, struct inpcb *, struct tdb *);
...
void	ipsec_set_mtu(struct tdb *, u_int32_t, uint64_t, const char *);
